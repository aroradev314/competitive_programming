...
void solve() {
	...
	vector<mint> dp(n / 2 + 2);
	dp[0] = (mint) 1;
	for (int i = 0; i < 26; i++) {
		if (c[i] == 0) continue;
		for (int j = n / 2 + 1 - c[i]; j >= 0; j--) {
			dp[j + c[i]] += dp[j];
			// if (j + c[i] == n - n / 2) cout << j << " " << dp[j] << endl;
		}
	}

	mint ans = fact[n / 2] * fact[n - n / 2] * dp[n - n / 2];
	for (int i = 0; i < 26; i++) ans = ans * inv_fact[c[i]];
